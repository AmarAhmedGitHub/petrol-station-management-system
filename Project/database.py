...
def create_table(): 
   # جدول أوامر التوريد المخزني
   c.execute('''CREATE TABLE IF NOT EXISTS FuelSupply (
       Supply_ID INT AUTO_INCREMENT PRIMARY KEY,
       Supply_Invoice_No VARCHAR(20) NOT NULL,
   ...
   # جدول دليل الطرمبة (يربط الطرمبة بالخزان والموظف)
   c.execute('''CREATE TABLE IF NOT EXISTS PumpDirectory (
       ID INT AUTO_INCREMENT PRIMARY KEY,
       Petrolpump_No VARCHAR(10) NOT NULL,
       FuelTank_ID VARCHAR(10),
   ...
   # تعديل جدول الطرمبة ليحتوي على FuelTank_ID
...
def add_Invoice_data(Invoice_No, Date, Payment_Type, Fuel_Amount, Fuel_Type, Discount, Total_Price, Customer_Code, Petrolpump_No=None, FuelTank_ID=None, Fuel_Type_Actual=None):
    # إضافة الحقول الجديدة إذا كانت موجودة في الجدول
    try:
        c.execute('INSERT INTO Invoice (Invoice_No, Date, Payment_Type, Fuel_Amount, Fuel_Type, Discount, Total_Price, Customer_Code, Petrolpump_No, FuelTank_ID, Fuel_Type_Actual) VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)',
                  (Invoice_No, Date, Payment_Type, Fuel_Amount, Fuel_Type, Discount, Total_Price, Customer_Code, Petrolpump_No, FuelTank_ID, Fuel_Type_Actual))
    except Exception:
        # إذا لم تكن الحقول موجودة، أضف الفاتورة بالطريقة القديمة
        c.execute('INSERT INTO Invoice  (Invoice_No , Date  , Payment_Type , Fuel_Amount , Fuel_Type , Discount  , Total_Price , Customer_Code) Values (%s,%s,%s,%s,%s,%s,%s,%s)',
                  (Invoice_No , Date  , Payment_Type , Fuel_Amount , Fuel_Type , Discount  , Total_Price , Customer_Code))
    mydb.commit()
