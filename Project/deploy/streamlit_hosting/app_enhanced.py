...
def main():
    ...
    if st.sidebar.button("نسيت كلمة المرور؟", key="reset_pw_btn"):
        ...
        if st.button("تغيير كلمة المرور"):
            ...
            if reset_type == "Owner":
                ...
                if row and row[0] and security_answer.strip().lower() == row[0].strip().lower():
                    hashed = bcrypt.hashpw(new_pw.encode('utf-8'), bcrypt.gensalt()).decode('utf-8')
                    database.c.execute("UPDATE Employees SET hashed_password=%s WHERE Emp_Name=%s", (hashed, reset_user))
                    database.mydb.commit()
                    st.success("تم تغيير كلمة المرور بنجاح. يمكنك الآن تسجيل الدخول.")
                else:
                    st.error("إجابة سؤال الأمان غير صحيحة أو المستخدم غير موجود.")
        ...
        with st.form("login_form", clear_on_submit=False):
            ...
            if login_btn:
                if st.session_state.locked_until and now < st.session_state.locked_until:
                    ...
                    st.stop()
                # Delegate authentication to central AuthManager (uses bcrypt + migration)
                auth_mgr = get_auth_manager()
                success, message = auth_mgr.handle_login(username, password)
                if success:
                    st.success(f"تم تسجيل الدخول: {message}")
                    st.rerun()
                else:
                    # message already contains attempt/lockout info
                    st.error(message)

    # After login - Using centralized layout with enhanced navigation
    from core.layout import (
        render_app_shell, render_welcome_message, render_sidebar_menu,
        render_breadcrumbs, render_page_header, render_section_nav
    )
    
    render_app_shell("نظام إدارة محطة الوقود")
    render_sidebar_menu(st.session_state.user_type, st.session_state.username)
    render_welcome_message(st.session_state.user_type, st.session_state.username)
    
    # Get active section from URL params or default
    query_params = st.experimental_get_query_params()
    active_section = query_params.get("section", ["dashboard"])[0]
    
    # Define main sections based on user type
    sections = [
    ...
    render_breadcrumbs("لوحة التحكم")
    render_section_nav(sections, active_section)
    
    # Render page header based on active section
    section_titles = {
        "dashboard": ("لوحة التحكم الرئيسية", "إدارة ومراقبة جميع عمليات محطة الوقود", "dashboard"),
        "reports": ("التقارير والإحصائيات", "عرض وتحليل تقارير المبيعات والأداء", "bar-chart"),
        "inventory": ("إدارة المخزون", "متابعة مستويات الوقود والمخزون", "box"),
        "employees": ("إدارة الموظفين", "إدارة حسابات وصلاحيات الموظفين", "people"),
        "settings": ("إعدادات النظام", "تخصيص وضبط إعدادات النظام", "gear"),
        "sensors": ("إدارة المستشعرات", "مراقبة وضبط المستشعرات", "activity")
    }
    
    title, desc, icon = section_titles.get(active_section, ("لوحة التحكم", "", "dashboard"))
    render_page_header(title, desc, icon)

    # Enhanced main sections with active section context
