...
def create_shifts_tab():
    ...
    if shifts:
        ...
        for shift in shifts:
            # Convert times to datetime objects for calculation
            start_dt = datetime.combine(datetime.today(), shift[1])
            end_dt = datetime.combine(datetime.today(), shift[2])
...
def shift_statistics_tab():
    ...
    for shift in shifts:
        # Convert times to datetime objects for calculation
        start_dt = datetime.combine(datetime.today(), shift[2])
        end_dt = datetime.combine(datetime.today(), shift[3])
