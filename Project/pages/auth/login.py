...
def authenticate_user(username, password):
    ...
    emp = c.fetchone()
    if emp:
        # Get employee permissions (simplified for now)
        perms = ["dashboard", "management", "reports", "sensor_monitoring", "accounting"]
        conn.close()
...
def handle_login(username, password):
    ...
    user_type, permissions = authenticate_user(username, password)
    ...
    if user_type:
        ...
        if st.session_state.login_attempts >= 5:
            st.session_state.locked_until = datetime.datetime.now() + datetime.timedelta(minutes=2)
            return False, "تم قفل الحساب مؤقتاً لمدة دقيقتين بسبب تكرار المحاولات الخاطئة."
        return False, f"بيانات الدخول غير صحيحة! (محاولة {st.session_state.login_attempts}/5)"
