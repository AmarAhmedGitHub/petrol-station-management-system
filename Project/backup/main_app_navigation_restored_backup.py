...
from pages.management.main_management_navigation_fixed import main as management_main
from pages.reports.main_reports_navigation_fixed import main as reports_main
from pages.admin.main_admin import main as admin_main
from pages.sensor_monitoring import main as sensor_monitoring_main
...
def check_user_permissions(user_type, required_permission):
    ...
    permissions = {
        'Employee': ['dashboard', 'management', 'reports', 'sensor_monitoring']
    }
    ...
    if not st.session_state.get('logged_in', False):
        ...
        st.markdown(f"""
            <div class="navigation-info">
                <strong>🧭 معلومات التنقل:</strong><br>
                • الصفحة الحالية: {st.session_state.current_page}<br>
                • الصفحة السابقة: {st.session_state.previous_page}<br>
                • نوع المستخدم: {user_type}<br>
                • الصفحات المتاحة: {list(available_pages.keys())}
            </div>
        """, unsafe_allow_html=True)
